...
class parser_dynamic_function
{
   public:
    ...
    template <typename Context>
    static inline expression_node_ptr parse_string_function_call(Context& ctx,
                                                                 igeneric_function<T>* function,
                                                                 const std::string& function_name)
    {
        ctx.next_token();
        ...
        if (!parse_igeneric_function_params(ctx, param_type_list, arg_list, function_name, function,
                                            tc))
        {
            return Context::error_node();
        }
        ...
        expression_node_ptr result =
            (tc.paramseq_count() <= 1)
                ? ctx.string_function_call(function, arg_list)
                : ctx.string_function_call(function, arg_list, param_seq_index);
    ...
    template <typename Context>
    static inline expression_node_ptr parse_overload_function_call(Context& ctx,
                                                                   igeneric_function<T>* function,
                                                                   const std::string& function_name)
    {
        ctx.next_token();
        ...
        typename Context::type_checker_t tc(ctx.parser_, function_name,
                                            function->parameter_sequence,
                                            Context::type_checker_t::e_overload);
        ...
        if (Context::type_checker_t::e_numeric == tc.return_type(param_seq_index))
        {
            result = (tc.paramseq_count() <= 1)
                         ? ctx.generic_function_call(function, arg_list)
                         : ctx.generic_function_call(function, arg_list, param_seq_index);
        }
        else if (Context::type_checker_t::e_string == tc.return_type(param_seq_index))
        {
            result = (tc.paramseq_count() <= 1)
                         ? ctx.string_function_call(function, arg_list)
                         : ctx.string_function_call(function, arg_list, param_seq_index);
