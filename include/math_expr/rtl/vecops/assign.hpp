...
#ifndef MATH_EXPR_RTL_VECOPS_ASSIGN_HPP
#define MATH_EXPR_RTL_VECOPS_ASSIGN_HPP

#include "math_expr/rtl/vecops/details/helper.hpp"

namespace math_expr::rtl::vecops
{
template <typename T> class assign final : public math_expr::igeneric_function<T>
{
  public:
    ...
    assign() : math_expr::igeneric_function<T>("VT|VTTT|VTTTT")
    /*
       Overloads:
       0. VT    - vector, V
    ...
    */
    ...
    inline T operator()(const std::size_t& ps_index, parameter_list_t parameters) override
    {
        ...
        const std::size_t step_size =
            (2 != ps_index) ? 1 : static_cast<std::size_t>(scalar_t(parameters.back())());

        std::size_t r0 = 0;
        std::size_t r1 = vec.size() - 1;

        if (((ps_index == 1) || (ps_index == 2)) &&
            !details::load_vector_range<T>::process(parameters, r0, r1, 2, 3, 0))
        {
            return T(0);
        }
