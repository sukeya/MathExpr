...
class select final : public math_expr::igeneric_function<T>
{
   public:
    ...
    select() : math_expr::igeneric_function<T>("VVVV|VVVVTT")
    /*
       Overloads:
    ...
    */
    ...
    inline T operator()(const std::size_t& ps_index, parameter_list_t parameters) override
    {
        ...
        if (1 == ps_index)
        {
            ...
            if (details::invalid_range(out, r0, r1))
                return T(0);
            if (details::invalid_range(mask, r0, r1))
                return T(0);
            if (details::invalid_range(vec0, r0, r1))
                return T(0);
            if (details::invalid_range(vec1, r0, r1))
                return T(0);
        }
