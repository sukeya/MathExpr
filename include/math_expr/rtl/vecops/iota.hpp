...
{
template <typename T> class iota final : public math_expr::igeneric_function<T>
{
  public:
    ...
    iota() : math_expr::igeneric_function<T>("VTT|VT|VTTTT|VTTT")
    /*
       Overloads:
       0. VTT  - vector, SV, SS
       1. VT   - vector, SV, SS (+1)
    ...
    */
    ...
    inline T operator()(const std::size_t& ps_index, parameter_list_t parameters) override
    {
        ...
        const T start_value = (ps_index <= 1) ? scalar_t(parameters[1]) : scalar_t(parameters[3]);

        const T step_size =
            ((0 == ps_index) || (2 == ps_index)) ? scalar_t(parameters.back())() : T(1);

        std::size_t r0 = 0;
        std::size_t r1 = vec.size() - 1;

        if (((ps_index == 2) || (ps_index == 3)) &&
