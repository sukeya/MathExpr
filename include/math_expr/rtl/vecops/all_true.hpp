...
#ifndef MATH_EXPR_RTL_VECOPS_ALL_TRUE_HPP
#define MATH_EXPR_RTL_VECOPS_ALL_TRUE_HPP

#include "math_expr/rtl/vecops/details/helper.hpp"

namespace math_expr::rtl::vecops
{
template <typename T> class all_true final : public math_expr::igeneric_function<T>
{
  public:
    ...
    all_true() : math_expr::igeneric_function<T>("V|VTT|T*")
    /*
       Overloads:
       0. V   - vector
       1. VTT - vector, r0, r1
    ...
    */
    ...
    inline T operator()(const std::size_t& ps_index, parameter_list_t parameters) override
    {
        if (2 == ps_index)
        {
            ...
            {
                if (math_expr::core::numeric::details::is_false_impl(scalar_t(parameters[i])()))
                {
        ...
        else
        {
            ...
            {
                if (math_expr::core::numeric::details::is_false_impl(vec[i]))
                {
                    return math_expr::core::numeric::false_v<T>;
                }
            }
