...
class generator
{
   public:
    ...
    {
        if (!token_list_.empty() && (token_list_.end() != token_itr_))
        {
    ...
    {
        const core::char_cptr begin_itr =
            ((base_itr_ + begin) < s_end_) ? (base_itr_ + begin) : s_end_;
   ...
   private:
    ...
    {
        return ((itr + 1) != s_end_) && core::is_digit(*(itr + 1));
    }
    ...
    inline void scan_operator()
    {
        ...
        if (!is_end(s_itr_ + 1))
        {
            ...
            if ((c0 == '<') && (c1 == '='))
                ...
                ttype = token_t::e_addass;
            else if ((c0 == '-') && (c1 == '='))
                ttype = token_t::e_subass;
            else if ((c0 == '*') && (c1 == '='))
                ttype = token_t::e_mulass;
            else if ((c0 == '/') && (c1 == '='))
                ttype = token_t::e_divass;
            else if ((c0 == '%') && (c1 == '='))
                ttype = token_t::e_modass;
    ...
    inline void scan_symbol()
    {
        ...
        while (!is_end(s_itr_))
        {
            if (!core::is_letter_or_digit(*s_itr_) && ('_' != (*s_itr_)))
            {
                ...
                */
                if ((s_itr_ != initial_itr) && !is_end(s_itr_ + 1) &&
                    !core::is_letter_or_digit(*(s_itr_ + 1)) && ('_' != (*(s_itr_ + 1))))
                    break;
            }
    ...
    inline void scan_special_function()
    {
        ...
        {
            t.set_error(token::e_err_sfunc, initial_itr, std::min(initial_itr + 11, s_end_),
                        base_itr_);
            token_list_.push_back(t);
        ...
        if (!(('$' == *s_itr_) && (core::imatch('f', *(s_itr_ + 1))) &&
              (core::is_digit(*(s_itr_ + 2))) && (core::is_digit(*(s_itr_ + 3)))))
        {
            t.set_error(token::e_err_sfunc, initial_itr, std::min(initial_itr + 4, s_end_),
                        base_itr_);
            token_list_.push_back(t);
