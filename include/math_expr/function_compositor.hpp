namespace math_expr
{
   ...
   private:
      ...
      struct base_func : public math_expr::ifunction<T>
      {
         ...
         {
            math_expr_assign(0) math_expr_assign(1)
            math_expr_assign(2) math_expr_assign(3)
         }
                      ...
                      const bool override = false)
      {
         ...
         if (expr_map_.end() != itr)
         {
            if (!override)
            {
               math_expr_debug(("Compositor error(add): function '%s' already defined\n",
                             name.c_str()));
         ...
         {
            math_expr_debug(("Compositor error(add): Failed to compile function '%s'\n",
                          name.c_str()));
                              ...
                              bool  return_present = false)
      {
         ...
         if (parser_.dec().return_present())
         {
            ...
            for (std::size_t i = 0; i < ret_param_list.size(); ++i)
            {
               ...
               {
                  math_expr_debug(("Compositor Error: Return statement in function '%s' is invalid\n",
                                name.c_str()));
         ...
         {
            math_expr_debug(("Compositor Error: Failed to add function '%s' to symbol table\n",
                          name.c_str()));
            return false;
         }
      }
